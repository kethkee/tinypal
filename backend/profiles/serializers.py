...
class TaskSerializer(serializers.Serializer):
    ...
    category = serializers.ChoiceField(
        choices=sorted(TASK_CATEGORIES)
    )
...
class CommitmentSerializer(serializers.Serializer):
    ...
    category = serializers.ChoiceField(
        choices=sorted(COMMITMENT_CATEGORIES)
    )
    ...
    def validate_title(self, value):
        ...
        if not value:
            raise serializers.ValidationError(
                "A commitment title is required."
            )
...
class ProfileSerializer(
    ...
    def validate_preferred_study_times(
        self,
        value,
    ):
    ...
    def validate_daily_study_target(
        self,
        value,
    ):
        ...
        if value < 1:
            raise serializers.ValidationError(
                "Daily study target must be positive."
            )
    ...
    def create(
        ...
        validated_data["tasks"] = [
            ...
            for task in validated_data.get(
                "tasks",
                [],
            )
        ]

        # Keep legacy single-value field
        # synchronized with the first selected window.
        preferred_times = (
            validated_data.get(
